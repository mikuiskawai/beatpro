...
let nextNoteIndex = 0;
let gameStartTime = 0;
const noteTravelDuration = 7200 / (parseInt(localStorage.getItem('speed')) || 8); // Time for the note to travel down (milliseconds)
...
function parseNoteTimings(lines) {
    ...
    let startSyncTime = syncTime + parseInt(localStorage.getItem('offset'));
...
function endGame() {
    ...
    }else if (finalScore >= 750000) {//761
        message = 'GODLY!';
    }else if (finalScore >= 650000 && finalScore < 750000) {//761
        message = 'LEDGENDARY!';
    }else if (finalScore >= 500000 && finalScore < 650000) {//761
        message = 'EXCELENT!';
    } else if (finalScore >= 350000 && finalScore < 500000) {//761
        message = 'WELL DONE!';
    }else if (finalScore >= 250000 && finalScore < 350000) {//761
        message = 'GREAT!';
    }else if (finalScore >= 200000 && finalScore < 250000) {//761
        message = 'GOOD!';
    } else if (finalScore >= 100000 && finalScore < 200000) {
        message = 'NOT BAD!';
    } else if (finalScore >= 50000 && finalScore < 100000) {
        message = 'Bad!';
    } else {
        message = 'Fail!';
    }
...
function playAnimation(elementId, animationName) {
    const animatedDiv = document.getElementById(elementId);
    animatedDiv.style.animation = '';
    animatedDiv.style.animation = `${animationName} 0.2s forwards`;
...
function createStar() {
    ...
    star.addEventListener('animationend', () => {
        ...
        let starspeed = (10 - 0.1) * Math.exp(-0.02 * combo) + 0.1
            // 랜덤한 크기와 방향 설정
    const size = Math.random() * 5 + 1;
    const angle = Math.random() * 2 * Math.PI;
    const distance = 800; // 중앙에서 퍼져나가는 거리 설정
...
function startStarCreation() {
    createStar(); // 최초 1초에 별 생성
    setTimeout(startStarCreation, 500); // 1초마다 다시 생성
}

startStarCreation(); // 첫 번째 실행
*/